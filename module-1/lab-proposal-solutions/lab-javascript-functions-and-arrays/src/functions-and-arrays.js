...
function uniquifyArray(array) {
  ...
  const uniqifiedArray = [];

  for (let item of array) {
    if (!uniqifiedArray.includes(item)) {
      uniqifiedArray.push(item);
    }
  }
...
function greatestProductDiagonal(matrix) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    for (let j = 0; j < matrix[i].length; j++) {
      // Greatest product in a diagonal ltr (left to right)
      if (i < matrix.length - 3 && j < matrix[i].length - 3) {
        const product = matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3];

        if (product > greatestProduct.value) {
          greatestProduct.value = product;
          greatestProduct.direction = 'ltr';
        }
      }

      // Greatest product diagonal rtl (right to left)
      if (i < matrix.length - 3 && j - 3 >= 0) {
        const product = matrix[i][j] * matrix[i + 1][j - 1] * matrix[i + 2][j - 2] * matrix[i + 3][j - 3];
        if (product > greatestProduct.value) {
          greatestProduct.value = product;
          greatestProduct.direction = 'rtl';
        }
      }
    }
  }
